...
import com.tubeplus.board_service.posting.port.out.VotePersistent.FindVoteDto;
...
public class PostingViewInfo {
    ...
    private static Vote getUserVote(Posting posting, String userUuid, VotePersistent votePersistence) {

        FindVoteDto findVoteDto
                = new FindVoteDto(posting.getId(), userUuid);

        Optional<Vote> optionalUserVote
                = votePersistence.findVote(findVoteDto)
                .ifExceptioned
                .thenThrow(ErrorCode.FIND_ENTITY_FAILED);

        log.info("in uservote");
        Vote userVote
                = optionalUserVote.orElseThrow(
